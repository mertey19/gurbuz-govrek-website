...
export function Events({ content }: { content: ManagedEventsContent }) {
  return (
    <section id="etkinlikler" className="relative isolate overflow-hidden bg-navy text-white">
      ...
      <Container className="section-space">
        <Reveal className="max-w-2xl">
          <SectionTitle eyebrow={content.eyebrow} title={content.title} description={content.description} tone="light" />
          <ul className="mt-8 grid gap-3 sm:grid-cols-2">
            {content.items.map((event) => <li key={event} className="flex gap-3 text-sm text-white/74"><Check className="mt-0.5 size-4 shrink-0 text-gold-light" aria-hidden="true" />{event}</li>)}
          </ul>
